...
loc2.add_action(Action('Bash the tower door', go('0x03')))

loc3 = Location('0x03', 'All right, you won.\nNo more nasty reboots\nIt\'s dark')
loc3.add_action(Action('Cast illumination spell', call('zx_reboot'), go('0x04')))
loc3.add_action(Action('Strike a match', call('zx_reboot'), go('0x04')))
loc3.add_action(Action('Wait until your eyes adjust', call('zx_reboot'), go('0x04')))
...
loc6 = Location('0x12', 'You see a telephone\nWhat number would you call?')
...
locF2 = Location('GLITCH', "Not so fast!\nCHIP8 DOES NOT HAVE\nA RESET ROUTINE! HAHAHA", id='final_reset')
...
DO IT NOW COWARD!
You don't dare, do you? :)""", id='final_close')
